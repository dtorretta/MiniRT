...
static float	calculate_distance_2(t_cylinder *cylinder,
	t_ray ray, t_quadratic *qdtc)
{
	float	dist3;
	float	closest;
	...
	if (qdtc->dist1 >= 0 && check_height(cylinder, ray, qdtc->dist1))
		closest = qdtc->dist1;
	if (qdtc->dist2 >= 0 && check_height(cylinder, ray, qdtc->dist2)
		&& qdtc->dist2 < closest)
		closest = qdtc->dist2;
...
static t_vector	calculate_normal(t_cylinder *cylinder, t_figure *closest)
{
	...
	projection = ft_scale(&cylinder->normal,
			ft_dot(&intersection, &cylinder->normal));
